...
ros::Publisher arduino_heading_pub;

void GPSHeadingAngleCallback(const std_msgs::Float32& msg)
{
	gps_heading_angle = msg.data;   // radian 으로 받을 것
	std_msgs::Float32 arduino_msg;
    	arduino_msg.data = msg.data;  // 받은 값을 그대로 Arduino로 전송
    	arduino_heading_pub.publish(arduino_msg);
}
...
void init_waypoint_region(void)
{
	...
	fp= fopen("//home//hi//catkin_ws//waypoints_files//0315.txt","r");
...
void camera_obs_detect_Callback(const std_msgs::Int16& msg)
{
    	camera_obs = msg.data;
}
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher car_total_pub = n.advertise<geometry_msgs::Twist>("/teleop_cmd_vel",1);
  arduino_heading_pub = n.advertise<std_msgs::Float32>("arduino_heading_angle", 1);
  ...
  ros::Subscriber camera_obs_flag_sub = n.subscribe("detected_objects",5, &camera_obs_detect_Callback);
  ...
  {	
    ...
    if(run_flag == 1)
    {
	  ...
	  //printf("%d %d \n", wp_go_id, no_waypoints);
			...
			//ROS_INFO("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
			ROS_INFO("CAR_STEER_ANGLE = %d", car_steering_angle);
	    ...
	    cmd_vel.linear.x = c_speed.data;  
	    cmd_vel.angular.z = static_cast<double>(s_angle.data);
	    //cmd_vel.angular.z = static_cast<double>(car_target_angle.data);
	    if(count>=2)
	    {
			//car_control_pub2.publish(c_speed);
	        //car_control_pub1.publish(s_angle);
	        //car_control_pub3.publish(car_target_angle);
	        car_total_pub.publish(cmd_vel);
	    }
